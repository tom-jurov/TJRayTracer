...
template <class T, unsigned int ROWS, unsigned int COLS>
T &TJRayTracer::MatrixXd<T, ROWS, COLS>::operator()(unsigned int row,
                                                    unsigned int col) {
...
template <class T, unsigned int ROWS, unsigned int COLS>
bool TJRayTracer::MatrixXd<T, ROWS, COLS>::operator==(
    const MatrixXd<T, ROWS, COLS> &other) {
  bool flag = true;
  if ((this != nullptr) && (&other != nullptr) && (ROWS == other.GetRows()) &&
      (COLS == other.GetCols())) {
...
template <class T, unsigned int ROWS, unsigned int COLS>
TJRayTracer::MatrixXd<T, ROWS, COLS>
TJRayTracer::MatrixXd<T, ROWS, COLS>::operator*(
    const MatrixXd<T, ROWS, COLS> &other) const {
...
template <class T, unsigned int ROWS, unsigned int COLS>
TJRayTracer::Point TJRayTracer::MatrixXd<T, ROWS, COLS>::operator*(
    const TJRayTracer::Point &other) const {
  TJRayTracer::Point result;
  result.x = _data[0 + 0 * COLS] * other.x + _data[1 + 0 * COLS] * other.y +
             _data[2 + 0 * COLS] * other.z + _data[3 + 0 * COLS] * other.w;
  result.y = _data[0 + 1 * COLS] * other.x + _data[1 + 1 * COLS] * other.y +
             _data[2 + 1 * COLS] * other.z + _data[3 + 1 * COLS] * other.w;
  result.z = _data[0 + 2 * COLS] * other.x + _data[1 + 2 * COLS] * other.y +
             _data[2 + 2 * COLS] * other.z + _data[3 + 2 * COLS] * other.w;
  result.w = _data[0 + 3 * COLS] * other.x + _data[1 + 3 * COLS] * other.y +
             _data[2 + 3 * COLS] * other.z + _data[3 + 3 * COLS] * other.w;
  return result;
}
...
template <class T, unsigned int ROWS, unsigned int COLS>
TJRayTracer::MatrixXd<T, ROWS, COLS>::MatrixXd(
    const TJRayTracer::MatrixXd<T, ROWS, COLS> &other) {
  for (std::size_t i = 0; i < ROWS; ++i) {
    for (std::size_t j = 0; j < COLS; ++j) {
...
template <class T, unsigned int ROWS, unsigned int COLS>
double TJRayTracer::MatrixXd<T, ROWS, COLS>::minor(unsigned int row,
                                                   unsigned int col) {
  return this->template submatrix<double, ROWS - 1, COLS - 1>(row, col)
      .Determinant();
}

template <class T, unsigned int ROWS, unsigned int COLS>
double TJRayTracer::MatrixXd<T, ROWS, COLS>::cofactor(unsigned int row,
                                                      unsigned int col) {
  if (COLS == 4) {
...
template <class T, unsigned int ROWS, unsigned int COLS>
TJRayTracer::MatrixXd<T, ROWS, COLS>
TJRayTracer::MatrixXd<T, ROWS, COLS>::inverse() {
...
template <class T, unsigned int ROWS, unsigned int COLS>
TJRayTracer::Vector TJRayTracer::MatrixXd<T, ROWS, COLS>::operator*(
    const TJRayTracer::Vector &other) const {
  Vector result;
  result.x = _data[0 + 0 * COLS] * other.x + _data[1 + 0 * COLS] * other.y +
             _data[2 + 0 * COLS] * other.z + _data[3 + 0 * COLS] * other.w;
  result.y = _data[0 + 1 * COLS] * other.x + _data[1 + 1 * COLS] * other.y +
