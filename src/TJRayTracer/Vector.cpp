...
TJRayTracer::Vector::Vector(double x, double y, double z)
    : Vec4(x, y, z, 0.0) {}
...
double TJRayTracer::Vector::dot(const TJRayTracer::Vector &lhs,
                                const TJRayTracer::Vector &rhs) {
  double result;
  if (!(&lhs == nullptr && &rhs == nullptr)) {
...
TJRayTracer::Vector TJRayTracer::Vector::cross(const TJRayTracer::Vector &lhs,
                                               const TJRayTracer::Vector &rhs) {
  Vector result;
  if (!(&lhs == nullptr && &rhs == nullptr)) {
...
TJRayTracer::Vector
TJRayTracer::Vector::reflect(const TJRayTracer::Vector &in,
                             const TJRayTracer::Vector &normal) {
